    ...
    $.ajax({
        ...
        if (data.length > 0) {
            for (let i = 0; i < data.length; i++) {
                ...
                // Verificar si el evento tiene un documento
                if (data[i]["com_documento"]) {
                    verEventoButton = `<a class="text-uppercase text-primary fw-bold" href="evento?id=${data[i]["comunicado_id"]}">VER EVENTO →</a>`;
                }

                // Limitar la descripción a 3 líneas o menos
                let descripcion = limitarTexto(data[i]["com_descripcion"], 3); // Limitar a 3 líneas
                ...
                <div class="col-lg-4 col-md-6 animated zoomIn">
                    <div class="blog-item bg-white rounded overflow-hidden shadow border h-100" 
                        ...
                        <div class="p-4">
                            ...
                            <p class="text-muted">${convertirLinks(limitarTexto(data[i]["com_descripcion"], 20))}</p>
            ...
            <div class="text-center mt-4">
                <a href="eventos" class="btn btn-lg btn-primary fw-bold px-4 py-2 rounded-pill shadow-lg"
                    style="background: linear-gradient(135deg, #007bff, #0056b3); border: none; transition: all 0.3s ease; display: inline-flex; align-items: center;"
                    ...
                    <i class="fa-solid fa-arrow-right me-2" style="transition: transform 0.3s ease;"></i> 
                    Más Eventos
                </a>
            </div>
            `;
...
function limitarTexto(texto, maxLines = 3) {
    let div = document.createElement('div');
    div.innerHTML = texto;
    let textoPlano = div.textContent || div.innerText || "";

    let lines = textoPlano.split(" ");
    if (lines.length > maxLines) {
        textoPlano = lines.slice(0, maxLines).join(" ") + "...";
    }
                ...
                <div class="col-lg-4 col-md-6 animated zoomIn" data-wow-delay="0.1s">
                    <div class="blog-item bg-white rounded overflow-hidden shadow border h-100"
                        ...
                        <div class="p-4">
                            ...
                            <p class="text-muted">${convertirLinks(limitarTexto(data[i]["ofi_descripcion"], 20))}</p>
...
function limitarTexto(texto, limite = 2) {
    // Convierte el HTML en texto sin etiquetas
    let div = document.createElement('div');
    div.innerHTML = texto;
    let textoPlano = div.textContent || div.innerText || "";

    // Si el texto es mayor que el límite
    if (textoPlano.split(" ").length > limite) {
