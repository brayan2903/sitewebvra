...
$('#tabla_comunicado').on('click', '.editar', function () {
    ...
    setTimeout(() => {
        if (CKEDITOR.instances['txt_desc_edit']) {
            CKEDITOR.instances['txt_desc_edit'].setData(data.com_descripcion);
        } else {
...
function Registrar_Comunicados() {
    ...
    // Convertir el contenido de CKEditor a texto normal
    let descripcion = CKEDITOR.instances['txt_desc'].getData();
    descripcion = descripcion.replace(/<[^>]*>/g, "");  // Elimina las etiquetas HTML, dejando solo el texto
    ...
    // Nombre de archivo para la imagen (si existe)
    ...
    let archivoobj = $("#txt_documento")[0].files[0];  // Documento PDF
    let archivoobjImg = $("#txt_img_prev")[0].files[0];  // Imagen
    ...
    if (archivoobj) formData.append("archivoobj", archivoobj);  // Solo agregar si existe el archivo
    if (archivoobjImg) formData.append("archivoobjImg", archivoobjImg);  // Solo agregar si existe la imagen
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (resp == 1) {
                // Muestra un mensaje mientras se suben los archivos
                Swal.fire({
                    title: 'Publicando...',
                    ...
                    if (result.dismiss === Swal.DismissReason.timer) {
                        // Al finalizar la carga, muestra el mensaje de éxito
                        ...
                        // Recargar la tabla de comunicados
...
function Modificar_Comunicado() {
    ...
    let descripcion = "";  // Inicializamos la variable descripcion

    // Verificar si la instancia de CKEditor está inicializada
    if (CKEDITOR.instances['txt_desc_edit']) {
        descripcion = CKEDITOR.instances['txt_desc_edit'].getData(); // Obtener el valor de CKEditor
        descripcion = descripcion.replace(/<[^>]*>/g, "");  // Eliminar etiquetas HTML
    } else {
        descripcion = document.getElementById("txt_desc_edit").value; // Si no usamos CKEditor, obtenemos el valor del textarea directamente
    }
    ...
    // Si hay un archivo PDF, se genera el nombre del archivo
    let nombrearchivo = "";
    if (documento.length > 0) {
        let extension = documento.split('.').pop();
        let f = new Date();
    ...
    // Generación del nombre de la imagen
    let extensionImg = img.split('.').pop();
    let nombrearchivoImg = "";
    let fImg = new Date();
    if (img.length > 0) {
    ...
    let formData = new FormData();
    let archivoobj = $("#txt_documento_edit")[0].files[0];  // Documento PDF
    let archivoobjImg = $("#txt_img_prev_edit")[0].files[0];  // Imagen
    ...
    // Agregar el archivo PDF solo si se seleccionó uno
    if (archivoobj) formData.append("archivoobj", archivoobj);

    // Agregar la imagen solo si se seleccionó una
    if (archivoobjImg) formData.append("archivoobjImg", archivoobjImg);
        ...
        success: function (resp) {
            if (resp == 1) {
                Swal.fire({
                    ...
                    if (result.dismiss === Swal.DismissReason.timer) {
                        Swal.fire("Mensaje de Confirmación", "El comunicado se modificó con éxito", "success");
                        tbl_comunicado.ajax.reload();
