...
<script>
// Variable global para las instancias de CKEditor
var editors = {};

function iniciarEditor(editorId, contenido = '') {
    if (typeof CKEDITOR !== 'undefined' && document.getElementById(editorId)) {
        ...
        // Establecer contenido si se proporciona
        editors[editorId].on('instanceReady', function () {
            this.setData(contenido);
        });
    }
}

// Inicializar CKEditor en el modal de registro
$('#modal_registro').on('shown.bs.modal', function () {
    iniciarEditor('txt_desc');
});

// Inicializar CKEditor en el modal de edición y cargar datos
$('#tabla_comunicado').on('click', '.editar', function () {
    ...
    $('#txt_id_edit').val(data.comunicado_id);
    $('#txt_titulo_edit').val(data.com_titulo);
    $('#select_estatus').val(data.com_estado).trigger('change');
    $('#imagen__prev_e').attr('src', "../" + data.com_img_prev);

    // Inicializar CKEditor con el contenido existente
    $('#modal_editar').on('shown.bs.modal', function () {
        iniciarEditor('txt_desc_edit', data.com_descripcion);
    }).modal('show');
});

// Manejar el envío del formulario de edición
function Modificar_Comunicado() {
    let idcom = $('#txt_id_edit').val();
    let titulo = $('#txt_titulo_edit').val();
    let descripcion = editors['txt_desc_edit'] ? editors['txt_desc_edit'].getData() : '';
    let estado = $('#select_estatus').val();

    // Validaciones y resto del código AJAX...
    console.log(descripcion); // Verificar que el contenido se está obteniendo
}

// Ocultar notificaciones no deseadas
                    ...
                    <div class="col-12 mb-3">
                        ...
                        <div class="tab-base">
                            ...
                            <ul class="nav nav-callout" role="tablist">
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" data-bs-toggle="tab" data-bs-target="#_dm-coTabsBaseProfile2" type="button" role="tab" aria-controls="profile" aria-selected="false">Documento Principal</button>
                                </li>
                            </ul>
                            <!-- Tabs content -->
